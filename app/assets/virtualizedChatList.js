 ...
 * VirtualizedChatList
 * ─────────────────────────────────────────────────────────────────────────────
 * DOM'a sadece görünür alandaki satırları render eder.
 * Kullanıcı sayısı ne kadar artarsa artsın tarayıcı donmaz.
 *
 * Kullanım:
 *   const vcl = new VirtualizedChatList(containerEl, { itemHeight: 73, overscan: 5 });
 *   vcl.setItems(items);           // items = { type:'chat'|'story'|'setting', data, ... }[]
 *   vcl.destroy();                 // scroll listener'ı temizler
 *
 * Mevcut renderChatsList / renderStoriesList / renderSettingsList fonksiyonlarını
 * bu sınıfla değiştirmek için aşağıdaki "Drop-in Replace" bloğunu kullan.
 */

class VirtualizedChatList {
    /**
     * @param {HTMLElement} container   – #chats-list elementi
     * @param {object}      options
     * @param {number}      options.itemHeight  – her satırın piksel yüksekliği (default 73)
     * @param {number}      options.overscan    – görünür alanın üst/alt dışında render edilecek ekstra satır (default 5)
     */
    constructor(container, options = {}) {
        ...
        this._items = [];       // tüm liste (ham veri)
        this._rendered = [];    // şu an DOM'daki { index, el } çiftleri
        this._scrollTop = 0;
        this._frameId = null;

        /* Sabit yükseklikli sartop spacer + liste sarmalayıcısı */
        this._setupDOM();
    ...
    _setupDOM() {
        ...
        /* Toplam yüksekliği temsil eden görünmez blok */
        this._phantom = document.createElement('div');
        this._phantom.style.cssText = 'position:absolute;top:0;left:0;width:1px;pointer-events:none;';
        this.container.appendChild(this._phantom);

        /* Gerçek item'ların yazıldığı alan */
    ...
    /**
     * Tüm item listesini ayarlar ve yeniden render eder.
     * @param {Array} items   – her eleman { type, data, render } içermelidir
     *   - type   : 'chat' | 'story' | 'setting' | 'empty' | 'loading'
     *   - data   : ham nesne (user, storyData, vb.)
     *   - render : (item) => HTMLElement  → DOM elementini döndüren fabrika fn
     */
    setItems(items) {
        this._items = items;
        this._clearRendered();
    ...
    /** Scroll listener'ı kaldırır, kaynakları serbest bırakır */
    destroy() {
    ...
    _onScroll() {
        ...
        if (this._frameId) return;                      // throttle – frame başına 1 render
    ...
    _render() {
        ...
        /* Görünür pencere (overscan dahil) */
        ...
        for (let i = startIdx; i <= endIdx; i++) {
            ...
            /* Pozisyonu sabitlemek için absolute */
        ...
        /* listEl kendi yüksekliğini de güncelle (absolute children için) */
...
/* ═══════════════════════════════════════════════════════════════════════════
 * Drop-in Replace – mevcut render fonksiyonlarının yerini alır
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * app.js'de şu satırı bir kez çalıştır (örn. initApp içinde):
 *
 *   const vcl = new VirtualizedChatList(document.getElementById('chats-list'));
 *   window._vcl = vcl;   // global erişim için
 *
 * Sonra renderChatsList, renderStoriesList, renderSettingsList'i aşağıdakilerle
 * TAMAMEN DEĞİŞTİR:
 */
        ...
        render() {
            ...
            el.innerHTML = `
                <div class="w-12 h-12 rounded-full flex items-center justify-center text-white font-medium shrink-0">
                    <img src="${user.profilePic || DEFAULT_PROFILE_PIC}"
                ...
                <div class="ml-3 flex-1 min-w-0">
                    <div class="flex justify-between">
                        <div class="font-medium truncate text-black dark:text-white">${escapeHtml(user.username)}</div>
                    </div>
                    <div class="text-sm text-gray-500 truncate">${escapeHtml(statusText)}</div>
                </div>`;
    ...
    return {
        ...
        render() {
            ...
            el.innerHTML = `
                ...
                <div class="ml-3 flex-1 min-w-0">
                    <div class="font-medium truncate text-black dark:text-white">${escapeHtml(user.username)}</div>
                    <div class="text-sm text-gray-500 truncate">${timeAgo(latestStory.createdAt)}</div>
                </div>`;
...
function makeEmptyItem(message) {
    return {
        ...
        render() {
            const el = document.createElement('div');
            el.className = 'flex items-center justify-center py-10 text-gray-500 dark:text-gray-400 text-sm';
...
function makeLoadingItem(message) {
    return {
        ...
        render() {
            const el = document.createElement('div');
            el.className = 'flex items-center justify-center py-10 text-gray-500 text-sm';
            el.textContent = message;
            return el;
        }
    };
...
/* ─── Yeni render fonksiyonları (eski olanların yerine koy) ─────────────── */
...
function renderChatsList() {
    ...
    const visibleUsers = state.allUsers.filter(u => u.socketId !== state.myId && !u.hidden);
    ...
    window._vcl.setItems(
        visibleUsers.map(u =>
            makeChatItem(u, { t, openChat, DEFAULT_PROFILE_PIC })
        )
    );
}
...
function renderStoriesList() {
    ...
    window._vcl.setItems(
        storyEntries.map(sd =>
            makeStoryItem(sd, { timeAgo, openStory, DEFAULT_PROFILE_PIC })
        )
    );
}
    ...
    const settings = [
        {
            ...
            label: t('copy_id'),
            onClick: () => { navigator.clipboard.writeText(state.myId); showToast(t('copied_id')); },
        },
        {
...
/* Search sonuçları için aynı pattern */
function renderChatSearchResults(users) {
    ...
    window._vcl.setItems(
        users.map(u => makeChatItem(u, { t, openChat, DEFAULT_PROFILE_PIC }))
    );
}
...
function renderStorySearchResults(stories) {
    ...
    window._vcl.setItems(
        stories.map(sd => makeStoryItem(sd, { timeAgo, openStory, DEFAULT_PROFILE_PIC }))
    );
}
...
/* changeLanguage'ı da virtualize et */
    ...
    const langs = [
        {
            ...
            onClick: () => { localStorage.setItem(STORAGE_KEYS.LANG, 'az'); currentLang = 'az'; translatePage(); },
        },
        {
            icon: `<img src="https://img.icons8.com/?size=64&id=J6RJcdGoJomQ&format=png">`,
            label: 'Türkçe',
            onClick: () => { localStorage.setItem(STORAGE_KEYS.LANG, 'tr'); currentLang = 'tr'; translatePage(); },
        },
        {
            icon: `<img src="https://img.icons8.com/?size=96&id=fIgZUHgwc76e&format=png">`,
            label: 'English',
            onClick: () => { localStorage.setItem(STORAGE_KEYS.LANG, 'en'); currentLang = 'en'; translatePage(); },
        },
        {
            icon: `<img src="https://img.icons8.com/?size=96&id=vioRCshpCBKv&format=png">`,
            label: 'Русский',
            onClick: () => { localStorage.setItem(STORAGE_KEYS.LANG, 'ru'); currentLang = 'ru'; translatePage(); },
        },
    ];
...
/*
 * ─── initApp içine EKLE (setupEventListeners çağrısından sonra) ───────────
 *
 ...
 *   renderChats();   // ilk render
 *
 * Bu kadar. Başka hiçbir şeyi değiştirmene gerek yok.
 */